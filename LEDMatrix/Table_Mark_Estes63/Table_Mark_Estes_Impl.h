...
void newpattern()//generates all the randomness for each new pattern
{
  ...
  if (!square && adjunct < 15 && adjunct > 1)
    adjunct = random(2, 6);
  dwell = 1000 * (random(TIMING , TIMING * 2.5)); //set how long the pattern will play
...
void audio3()// right channel
{
  for (int i = 0; i < 64 ; i ++)
    for (int j = 0; j < iscalerW; j++) {
      ...
      DLine(i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2 - 10, i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2,  CHSV(i * mstep  + blender - 85, 166, 255));

      DLine(i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2 - 5, i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2,  CHSV(i * mstep  + blender + 85, 128, 255));
    }
  audioprocess();
}
...
void swirl4a() {// outer
  ...
  {
    flop[9] = true;
    xhowmany = random(BIGGER * 2 / 3  , BIGGER * 4 / 3 );

    for ( int16_t i = 0; i < xhowmany; i++)
    {
      xslope[i] = random(BIGGER / 9, BIGGER / 4); //radius
      fpeed[i] = random(3, 22); //speed  bigger = slower
      fcolor[i] = blender + fpeed[i] * 11; //color
      fvelo[i] = random8();
      fcountr[i] = random8(); //position offset
    }
  }
  EVERY_N_SECONDS(25 + dot) {
    flop[9] = !flop[9];
    /* if (flop[9])
  ...
  for ( int16_t i = 0; i < xhowmany; i++) {
    if (flop[9]) {
      if (counter % int( fpeed[i]  ) == 0  && xslope[i] < BIGGER * 3 / 2 + 4 )
        xslope[i]++;
    }
    else if (counter % int( fpeed[i] ) == 0  && xslope[i] > BIGGER / 12)
      xslope[i]--;
    if (xhowmany > BIGGER) {
    ...
    {
      DFCircle(driftx + xslope[i] * (sin8(-h * fpeed[i] / 5 + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(-h * fpeed[i] / 5 + fcountr[i]) - 128.0) / 128, 5, CHSV(fcolor[i]  , 255 - fvelo[i] / 6, 255));
      DFCircle(driftx + xslope[i] * (sin8(-h * fpeed[i] / 5 + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(-h * fpeed[i] / 5 + fcountr[i]) - 128.0) / 128, 4, CHSV(fcolor[i]  + 128, 255 - fvelo[i] / 6, 255));
    }
  }
}
...
void art() {
  ...
  DFRectangle(0, 0, MATRIX_WIDTH, MATRIX_HEIGHT, 0x404064);//very dim white  96 is still dim
