...
#define mpatterns           (193)// max number of patterns
#define BallCount           30
...
EXTMEM uint8_t  mstep, LLaudio[MAXBIG], RRaudio[MAXBIG], inner, bfade = 3, passer = 0;
...
uint8_t acount, fpeed[MAXBIG] ,  iscalerW = int(scalerW + .1), iscalerH = int(scalerH + .1);
uint8_t  fcolor[MAXBIG], fcountr[MAXBIG];
uint8_t kind[BallCount], xpoffset[MAXBIG], fvelo[MAXBIG];
EXTMEM int16_t xvort[MAXBIG], yvort[MAXBIG];
EXTMEM uint16_t fcount[MAXBIG];

boolean square = false, gmusic =  false, flop[12],   nextsong = false;
EXTMEM boolean rimmer[MAXBIG], xbouncer[MAXBIG], ybouncer[MAXBIG];
...
EXTMEM float radius, xslope[MAXBIG], yslope[MAXBIG], xfire[MAXBIG], yfire[MAXBIG], cangle, sangle;
...
EXTMEM float ImpactVelocity[BallCount];
EXTMEM float TimeSinceLastBounce[BallCount];
EXTMEM int   Positiony[BallCount], Positionx[BallCount];
EXTMEM long  ClockTimeSinceLastBounce[BallCount];
EXTMEM float Dampening[BallCount];
