...
void newpattern()//generates all the randomness for each new pattern
{
  ...
  steper = random(1, 8);//color tempo
  shifty = random(2, 20); //how often the drifter moves (2,10)
  ccoolloorr = random8();
  blender = random8();
  xblender = random8();
  poffset = random8();
...
void checkshifty() {
  if (counter % shifty == 0) {
    ...
    if (driftx > (MATRIX_WIDTH - MIDLX / 4)) { //change directin of drift if you get near the right 1/4 of the screen
      cangle = 0 - fabs(cangle);
      driftx =  driftx + cangle;//move the x center every so often
    }
    if (driftx < MIDLX / 4) { //change directin of drift if you get near the right 1/4 of the screen
      cangle = fabs(cangle);
      driftx =  driftx + cangle;//move the x center every so often
    }
    drifty =  drifty + sangle;//move the y center every so often
    if (drifty > ( MATRIX_HEIGHT - MIDLY / 4)) { // if y gets too big, reverse
...
void checkfader() {
  switch (bfade)
  {
    ...
    case 110:
      ...
      break;
    default:
      lfado(16 + (bfade << 4)); // 1 = 16,  4 = 64, this sets the amont of fade as a fraction over 256  range is 16/256 (6.25%) i.e. not much  to 64/256 (25%)or considerable
      break;
  }
}
...
void Inca(byte brit)//eff 1
{
  ...
  if (flop[4])
    dot = beatsin8(3, 1, 4, 0);
  else
    dot = beatsin8(5, 1, 4, 0);
...
{
  if (flop[7] || flop[5] || flop[3])
    dot = beatsin8(dot2 , 1, 3, 0);
  for (int y = 0; y <= MATRIX_HEIGHT * 3 / 4  ; y += dot)
    if ( !flop[8] )
      DCircle(driftx, drifty , MATRIX_HEIGHT * 3 / 4 + 3 - y, CHSV(h  + y * steper, 255 - velo / 5, 255 - 2 * y));
    else
      DCircle(driftx, drifty , MATRIX_HEIGHT * 3 / 4 + 3 - y, CHSV(h  + y * steper, 255 -  y, 255));
...
void drifter()//pattern=5
{
  ...
  if (flop[7] || flop[5] )
    dot = beatsin8(dot2, 2, 4, 0);

  cool = beatsin8(dot2, 4, 12, blender);


  for (int y = 0; y <= MATRIX_HEIGHT  + 1 ; y += dot )
    DFCircle(driftx, drifty , MATRIX_HEIGHT  + 5 - y, CHSV(h + y * cool, 255 - velo / 5, 255));
}
...
void volcano(byte brit)//pattern=7
{
  ...
  for (int y = 0; y <  MATRIX_WIDTH + 8 ; y += (dot3))
    DCircle(driftx , drifty  ,  MATRIX_WIDTH + 4  - y, CHSV( h - y * 16, 255 - velo / 5, brit));
}
...
void pyrimid()//pattern=8
{
  ...
  dot = beatsin8(dot3 + dot2, 1, 4, 0);

  for (int y = 0; y < MATRIX_WIDTH; y += dot )
    DCircle(driftx, drifty,  y, CHSV(h  - y * 8, 255 - velo / 5, 255));
...
void solid5()//colors rotate forward
{
  for (int i = 0; i < MATRIX_WIDTH; i++)
    for (int j = 0; j < MATRIX_HEIGHT; j++)
      zeds(i, j) = CHSV(i * dot2 + dot / 2 * j +  h, 255, 128);
}
...
void fireworks() {// with colored dots in pairs sometimes
  ...
  if (flop[3]) {
    ...
    zeds(driftx, drifty) = CHSV(ccoolloorr - 96, 255 - velo / 5, 255);
  }
...
// full size waves overlapping with minimal fade...
{
  ...
  for (int jj = 0; jj < MATRIX_WIDTH; jj++) {
    ...
    for (int ii = 0; ii < 2 * dot2 + 3; ii++)
      zeds(jj , ii +  MIDLY + (sin8(3 * h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.70) += CHSV(h  - 64, 255 - velo / 5, 255);



    for (int ii = 0; ii < 3 * dot + 3; ii++)
      zeds( ii +  MIDLX + (sin8(-3 * h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.70, jj) += CHSV(h  + 64, 255 - velo / 5, 255);
  }
}
...
// horizontal waves
{
  ...
  for (int jj = 0; jj < MATRIX_WIDTH; jj++)
  {
    DLine(jj, (2 + MIDLY / 2 - (sin8(-2 * h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.50 ), jj, 0, CHSV(h  + 128, 255, 255));
    DLine(jj, (-2 + MIDLY * 3 / 2 - (sin8(h + jj * (sinewidth)) - 128) / 128.0 * MIDLY * 0.50 ), jj, MATRIX_HEIGHT, CHSV(h  , 255, 255));
  }
...
// horizontal waves
{
  ...
  for (int jj = 0; jj < MATRIX_WIDTH; jj++)
  {
    DLine(jj, (2 + MIDLY / 2 - (sin8(-3 * h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.50 ), jj, 0, CHSV(h + jj / 2 + 128, 255, 255));
    DLine(jj, (-2 + MIDLY * 3 / 2 - (sin8( h + jj * (sinewidth)) - 128) / 128.0 * MIDLY * 0.50 ), jj, MATRIX_HEIGHT, CHSV(h + jj / 2 , 255, 255));
  }
  ...
  for (int jj = 0; jj < MATRIX_WIDTH; jj++) {
    ...
    for (int ii = -dot2; ii < dot2 ; ii++)
      zeds(jj , ii +  MIDLY + (sin8(2 * h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.70) += CHSV(h + jj / 2 - 64, 255 - velo / 5, 255);

    for (int ii = -dot; ii <  dot ; ii++)
      zeds( ii +  MIDLX + (sin8(-2 * h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.70, jj) += CHSV(h + jj / 2 + 64, 255 - velo / 5, 255);
  }
}
...
void hypnoduck() {
  ...
  for (int jj = 0; jj < 360; jj += 5)
  {
    ...
    DFCircle(xxx + xangle * (jj /  17) , yyy + yangle * (jj / 17), dot, CHSV(poffset + h - 48, 255 - velo / 4, 255));

    xangle =  (sin8(jj + quash * h + 128) - 128.0) / 128.0;
    yangle =  (cos8(jj + quash * h + 128) - 128.0) / 128.0;
    DFCircle(xxx + xangle * (jj /  17) , yyy + yangle * (jj / 17), dot, CHSV(poffset + h + 48, 255 - velo / 5, 255));
  }
  ...
  for (int jj = 0; jj < 360; jj += 5)
  {
    ...
    xangle =  (sin8(jj + quash * h) - 128.0) / 128.0;
    yangle =  (cos8(jj + quash * h) - 128.0) / 128.0;
    DFCircle(xxx + xangle * (jj /  17) , yyy + yangle * (jj / 17), dot2, CHSV(poffset + h + 48 + 176, 255 - velo / 4, 255));
    xangle =  (sin8(jj + quash * h + 128) - 128.0) / 128.0;
    yangle =  (cos8(jj + quash * h + 128) - 128.0) / 128.0;
    DFCircle(xxx + xangle * (jj /  17) , yyy + yangle * (jj / 17), dot2, CHSV(poffset + h -  + 80, 255 - velo / 5, 255));
  }
...
void hypnoduck3()  // growing spirals
{
  ...
  if (flop[2] && flop[1])
    DFRectangle(0, 0, MATRIX_WIDTH  - 1, MATRIX_HEIGHT  - 1, CHSV(h + 85, 255 - velo / 5, 100));
  else {
    if ( flop[0])
      DFRectangle(0, 0, MATRIX_WIDTH  - 1, MATRIX_HEIGHT  - 1, CRGB::Black);
    else
  ...
  for (int jj = 20; jj < beatsin16(6, 190, 850, 0); jj += 5)
  {
    xangle =  (sin8(jj + quash * h) - 128.0) / 128.0;
    yangle =  (cos8(jj + quash * h) - 128.0) / 128.0;
    DFCircle( driftx + xangle * (jj /  (19 - dot)) , drifty + yangle * (jj / (19 - dot)), dot, CHSV(jj / 3 + h - 85, 255 - velo / 5, 255));
    xangle =  (sin8(jj + quash * h + 128) - 128.0) / 128.0;
    yangle =  (cos8(jj + quash * h + 128) - 128.0) / 128.0;
    DFCircle( driftx + xangle * (jj /  (19 - dot)) ,  drifty + yangle * (jj / (19 - dot)), dot, CHSV(jj / 3 + h , 255 - velo / 4, 255));
  }
}
...
// spirals with speckles of hyponic light random direction based on flop[1]
{
  ...
  DFCircle(driftx, drifty, dot, CHSV(-h + ccoolloorr + 85 , 255 - velo / 5, 255));
   xangle =  (sin8(0 + quash * h) - 128.0) / 128.0;
    yangle =  (cos8(0 + quash * h) - 128.0) / 128.0;
  DFCircle(driftx, drifty, dot, CHSV(-h + ccoolloorr - 85 , 255 - velo / 5, 255));
}
...
// spiral inward with the hyponic light
{
  ...
  for (int jj = beatsin16(4, 0, 950, 0) ; jj > 0; jj -= 5)//
  {
    xangle =  (sin8(jj + quash * -h) - 128.0) / 128.0;
    yangle =  (cos8(jj + quash * -h) - 128.0) / 128.0;
    DFCircle(driftx + xangle * (jj /  17) , drifty + yangle * (jj / 17), 2, CHSV(jj / 3 + h + 64, 255 - velo / 5, 255));
...
void starz()// 3 stars spin in a circle
{
  ...
  for (int i = 0; i < howmany; i++)
    for (int j = 2; j < inner; j += 1)
      ...
      {
        DALine(driftx + xcen + ((j * (sin8(i * radius2 + radius2 / 2 +  h) - 128.0)) / 128), drifty + ycen + ((j * (cos8(i * radius2 + radius2 / 2 +  h) - 128.0)) / 128), driftx + xcen + ((MIDLX * (sin8(i * radius2 +  h) - 128.0)) / 128), drifty + ycen + ((MIDLY * (cos8(i * radius2 +  h) - 128.0)) / 128), CHSV(h - 85 + dot  * j , 255 - velo / 4 , 255));
        DALine(driftx + xcen + ((j * (sin8(i * radius2 - radius2 / 2 +  h) - 128.0)) / 128), drifty + ycen + ((j * (cos8(i * radius2 - radius2 / 2 +  h) - 128.0)) / 128), driftx + xcen + ((MIDLX * (sin8(i * radius2 +  h) - 128.0)) / 128), drifty + ycen + ((MIDLY * (cos8(i * radius2 +  h) - 128.0)) / 128), CHSV(h - 85 + dot  * j , 255 - velo / 4 , 255));
      }
...
void wheelz()// circles and stars filled circles in a circle
{
  ...
  for (int i = 0; i < howmany; i++)
    if (i % 2 == 0) {
      DFCircle(driftx + (((inner) * (sin8(i * radius2 +  h) - 128.0)) / 128), drifty + (((inner) * (cos8(i * radius2 +   h) - 128.0)) / 128), inner , CHSV(h  + 12 * i , 180, 255));
      DCircle(driftx + ((inner * (sin8(i * radius2 +  h) - 128.0)) / 128), drifty + ((inner * (cos8(i * radius2 +   h) - 128.0)) / 128), inner , CHSV(h  + 12 * i , 255 - velo / 8, 255));
    }
    else
...
void wheelz2()// circles in a circle
{
  ...
  for (int i = 0; i < howmany; i++)
  { if (i % 2 == 0)
      DCircle(driftx + ((inner * (sin8(i * radius2 -  h) - 128.0)) / 128), drifty + ((inner * (cos8(i * radius2 -   h) - 128.0)) / 128), inner, CHSV(h  + 12 * i , 255 - velo / 5, 255));
    else
      drawstar(driftx + (((inner ) * (sin8(i * radius2 +  h) - 128.0)) / 128), drifty + (((inner ) * (cos8(i * radius2 +   h) - 128.0)) / 128), inner, inner / 2, dot2,  h, h  + 12 * i); // random multipoint star
  }
}
...
void drips() {
  ...
  for (int i = 0 ; i < howmany; i++) {//draw all the drops
    ...
    DCircle(xslope[i] , yslope[i] , dot, CHSV(fcolor[i], fvelo[i] / 4 + 192, 255)); //Draw a line from (x0, y0) to (x0, y0+2) The y+2 will make the raindrop 3 pixels tall
    if (i % MIDLX == 0)
      DCircle(xslope[i] , yslope[i] , dot, CRGB::White); //toss in a white one every so often
  }
}
...
void VORTEX() {
  ...
  if (flop[2])
    howmany = beatsin8(2, how / 2, how, 0);
  ...
  for (int i = 0 ; i < howmany; i++) {//draw all the drops
    ...
    {
      ADFCircle(xslope[i] , yslope[i], dot3 + 1,  CHSV(fcolor[i], fvelo[i] / 4 + 192, 255)); //
      if (i % (23) == 0)
