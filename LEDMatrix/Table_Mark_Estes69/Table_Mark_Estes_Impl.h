...
void newpattern()//generates all the randomness for each new pattern
{
  ...
  wind = random8(60);// 1 in 4 will have wind
  fancy = random8(100);//50 seemed to give too many fancys  now ~1 in 3 will do something
  dot = random(2, 6);// controls the size of a circle in many animations
  dot2 = random(2, 6);
  dot3 = random(1, 4);
  adjunct = random(45);//controls which screen wide effect is used if any.  this uses bigger value than number of adjuncts so allows the default to do naught.
  if (!square && adjunct < 15 && adjunct > 1)
    adjunct = random(2, 6);
  dwell = 1000 * (random(TIMING , TIMING * 1.7)); //set how long the pattern will play
  ringdelay = random(35, 80);//sets how often one of the effects will happen
  bringdelay = random(80, 200);//sets how often one of the effects will happen
...
void regenpattern() {
  ...
  dot2 = random(2, 6);
  dot3 = random(1, 4);
  adjunct = random(50);//controls which screen wide effect is used if any.  this uses bigger value than number of adjuncts so allows the default to do naught.
  if (!square && adjunct < 15 && adjunct > 1)
    adjunct = random(2, 6);
  dwell = 1000 * (random(TIMING , TIMING * 1.7)); //set how long the pattern will play
  ringdelay = random(35, 80);//sets how often one of the effects will happen
...
void tellit() {
  ...
  /*
    ...
    Serial.print(", gotmusic: ");
    Serial.println(gotmusic);
    #ifdef TME_AUDIO
    Serial.print(" Scale: " );
...
void checktempo() {
  if (counter % 8 == 0) {
    ...
    if (fps <= targetfps - 10 && waiter > 5)
      waiter -= 5;

    if (fps <= targetfps - 15 && waiter > 10)
      waiter -= 10;
  }
}
...
void whatami()// set some parameters specific to the pattern and send some data to the serial port for trouble shooting/ tweaking
{
  ...
  switch (pattern)
  {
    ...
    case 17:
      ...
      adjunct = 0;
      //targetfps = 15;
      bfade = random(118, 121);// the color fados
      break;
    case 18:
      adjunct = 0;
      fancy = 9;
    ...
    case 28:
      ...
      Serial.print("Raudio9 with wind: ");
      Serial.print(wind);
      targetfps = 30;
      slowme = false;//no slowmo.
      break;
    ...
    case 57:
      ...
      break;
    case 58:
      Serial.print("whitewarp no fancy ");
      //  adjunct = 0;
    ...
    case 162:
      ...
      Serial.print("seasick6 adjust");
      adjunct = random(1, 3) * 4;
      break;
    ...
    case 192:
      ...
      break;
    case 193:
      Serial.print("h noize far zoom");
      zoomer = 5;
      pscale = random(13, 18);//pscale = random(14, 20);
...
void Inca()//eff 1
{
  ...
  if (flop[4])
    dot = beatsin8(1, 1, 3, 0);
  else
    dot = beatsin8(3, 1, 3, 0);

  cool = beatsin8(dot2, 4, 6 , dot);
...
void solid()//colors rotate forward
{
  if (counter == 0)
    rr = random8();
  DFRectangle(0 , 0,  MATRIX_WIDTH - 1, MATRIX_HEIGHT - 1, CHSV(rr + beatsin8(3, 0, 96, 0), 255 - velo / 5, 120));
}
...
void solid5()//colors rotate forward
{
  for (int i = 0; i < MATRIX_WIDTH; i++)
    for (int j = 0; j < MATRIX_HEIGHT; j++)
      zeds(i, j) = CHSV(i * dot2 + dot / 2 * j +  2 * h, 255, 120);
}
...
void fuzzy() {  ///pattern 12
  ...
  zeds(random(MATRIX_WIDTH), random(MATRIX_HEIGHT)) = CHSV(h - 64, 255, 255);
  zeds(random(MATRIX_WIDTH), random(MATRIX_HEIGHT)) = CHSV( h + 32, 255 - velo / 2, 255 - velo / 8);
  zeds(random(MATRIX_WIDTH), random(MATRIX_HEIGHT)) = CHSV( h + 64, 255 - velo / 4, 255 - velo / 5);
  zeds(random(MATRIX_WIDTH), random(MATRIX_HEIGHT)) = CHSV( h , 255 - velo / 2, 255 - velo / 8);
  zeds(random(MATRIX_WIDTH), random(MATRIX_HEIGHT)) = CHSV( h - 32 + 128, 255 - velo / 2, 255 - velo / 7);
}
...
void spire3() {
  ...
  dot = beatsin8(2, 4, 10, 7);
  radius = beatsin16(5, BIGGER / 8, BIGGER / 2, 0);
  radius2 = beatsin16(5, BIGGER / 8, BIGGER / 2, BIGGER / 5);

  for (int i = 0; i < howmany; i++) {
    float xer = driftx + radius * (cos8(2 * h + i * 256 / howmany) - 128.0) / 128.0;
    float yer = drifty + radius * (sin8(2 * h + i * 256 / howmany) - 128.0) / 128.0;
    if (!flop[2]) {
      DFCircle(xer, yer, dot, CHSV(h + i * 256 / howmany, 255 - velo / 5, 148));
      DCircle(xer, yer, dot, CHSV( h + i * 256 / howmany, 255 - velo / 5, 205));
    }
    else {
      DFCircle(xer, yer, dot, CHSV(h + i * 256 / howmany, 255 - velo / 5, 128));
      DCircle(xer, yer, dot, CHSV(128 + h + i * 256 / howmany, 255 - velo / 5, 205));
    }
...
void spire5() {
  ...
  dot = beatsin8(3, 5, 11, 8);
  radius = beatsin16(4, BIGGER / 7, BIGGER / 2, 0);
  radius2 = beatsin16(6, BIGGER / 7, BIGGER / 2, BIGGER / 5);
  for (int i = 0; i < howmany; i++) {
    float xer = driftx + radius * (cos8(2 * h + i * 256 / howmany) - 128.0) / 128.0;
    float yer = drifty + radius * (sin8(2 * h + i * 256 / howmany) - 128.0) / 128.0;
    if (!flop[2])
      DFCircle(xer, yer, dot + 2, CHSV(h + i * 256 / howmany, 255 - velo / 5, 255));
    else {
    ...
    yer = drifty - radius2 * (sin8(2 * h + 128 / howmany + i * 256 / howmany) - 128.0) / 128.0;
    if (!flop[2])
      DFCircle(xer, yer, dot + 2, CHSV(h + 32 + i * 256 / howmany, 255 - velo / 5, 255));
    else {
...
void spire() {
  ...
  DFCircle(xer, yer, dot, CHSV(h + 84, 255 - velo / 5, 128));
  ...
  DFCircle(xer, yer, dot, CHSV(h + 42, 255 - velo / 5, 128));
...
void triple() {
  ...
  radius3 = beatsin8(3, howmany / 4 , howmany , 0);
...
void iris() {
  if (counter == 0) {
    howmany = random(3, 7);
    howmany = 2 + howmany * 2; //an even number
    for (int i = 0 ; i < howmany ; i++) {
      fvelo[i] =  random8();
      fcolor[i] = i * 256 / howmany;
      if (i % 2 == 0)
        fcolor[i] += 128;
    }
    radius2 = random(BIGGER / 4, BIGGER * 3 / 4);
    // radius3 = random(radius2 * 3 / 4  , radius2 * 3 / 2 );
    poffset = random(BIGGER / 10, BIGGER / 6);
    dot2 = 2;
    dot3 = 2;
    if (random8() > 128) dot2 = 1;
    if (random8() > 128) dot3 = 1;
  ...
  radius3 = beatsin16(5, radius2 + poffset / 2 , radius2 * 3 / 2 - poffset / 2, 0 );
  for (int i = 0 ; i < howmany ; i++) {
    float mcentrx = driftx + ((radius3 * (cos8(h * dot3  + i * 256 / howmany) - 128)) >> 7);
    float mcentry =  drifty + ((radius3 * (sin8(h * dot3 + i * 256 / howmany) - 128)) >> 7);
    float scentrx = driftx + ((radius3 * (cos8(-h * dot2  + i * 256 / howmany) - 128)) >> 7);
    float scentry =  drifty + ((radius3 * (sin8(-h * dot2   + i * 256 / howmany) - 128)) >> 7);
    for (int xxxx = 0; xxxx < MATRIX_WIDTH; xxxx++)
      for (int yyyy = 0; yyyy < MATRIX_HEIGHT; yyyy++)
        if (((xxxx - mcentrx) * (xxxx - mcentrx) + (yyyy - mcentry) * (yyyy - mcentry) < radius2 * radius2) && ((xxxx - scentrx) * (xxxx - scentrx) + (yyyy - scentry) * (yyyy - scentry) > radius2 * radius2))
          zeds(xxxx, yyyy) =  CHSV(fcolor[i] + h  , 255 - fvelo[i] / 2, 255);
...
void siny() {// 2 ring circus

  if (counter == 0) {
    dot++;
    radius2 = random(BIGGER / 4, BIGGER / 2) - 4;
    radius3 = random (BIGGER / 7, BIGGER / 4);
    ringdelay = ringdelay * random(2, 15);
  }
  if (flop[1]) {
    DCircle(MIDLX + ((radius3 * (cos8(-h ) - 128)) >> 7), MIDLY + ((radius3 * (sin8(-h ) - 128)) >> 7), radius2 + 1, CHSV(h * 2 - 64 , 255 - velo / 3, 255));
    DCircle(MIDLX + ((radius3 * (cos8(-h  + 128) - 128)) >> 7), MIDLY + ((radius3 * (sin8(-h  + 128) - 128)) >> 7), radius2 - 1, CHSV(h * 2 + 64 , 255 - velo / 3, 255));
  }
  else {
    DCircle(MIDLX + ((radius3 * (cos8(h ) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h ) - 128)) >> 7), radius2 + 1, CHSV(h * 2 - 64, 255 - velo / 3, 255));
    DCircle(MIDLX + ((radius3 * (cos8(h  + 128) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h  + 128) - 128)) >> 7), radius2 - 1, CHSV(h * 2 + 64 , 255 - velo / 3, 255));
...
void sinx() {// 3 ring circus

  if (counter == 0) {
    dot++;
    radius2 = random(BIGGER / 4, BIGGER / 2) - 4;
    radius3 = random (BIGGER / 7, BIGGER / 4);
    ringdelay = ringdelay * random(12, 25);
  }
  if (flop[1]) {
    DCircle(MIDLX + ((radius3 * (cos8(h ) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h ) - 128)) >> 7), radius2 + 1, CHSV(h * 2 - 64 , 255 - velo / 3, 255));
    DCircle(MIDLX + ((radius3 * (cos8(h  + 85) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h  + 85) - 128)) >> 7), radius2 - 1, CHSV(h * 2 + 21 , 255 - velo / 3, 255));
    DCircle(MIDLX + ((radius3 * (cos8(h  + 170) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h  + 170) - 128)) >> 7), radius2 - 1, CHSV(h * 2 + 106 , 255 - velo / 3, 255));
  }
  else
  {
    DCircle(MIDLX + ((radius3 * (cos8(h ) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h ) - 128)) >> 7), radius2 + 1, CHSV(h  + 100 , 255 - velo / 3, 255));
    DCircle(MIDLX + ((radius3 * (cos8(h  + 85) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h  + 85) - 128)) >> 7), radius2 - 1, CHSV(h * 2 + 100 , 255 - velo / 3, 255));
    DCircle(MIDLX + ((radius3 * (cos8(h  + 170) - 128)) >> 7), MIDLY + ((radius3 * (sin8(h  + 170) - 128)) >> 7), radius2 - 1, CHSV(h * 3 + 100 , 255 - velo / 3, 255));
...
void whitewarp() {
  if (counter == 0 )
  {
    ...
    for (int i = 0; i < howmany; i++) {
      ...
      fcolor[i] = blender + random8() / 4; //color
...
{
  if (counter == 0) {
    radius = mmin(( MATRIX_HEIGHT - 1 ) / 4, (MATRIX_WIDTH - 1) / 4) ;
  ...
  if (flop[2])
    DRectangle(0, 0, MATRIX_WIDTH - 1, MATRIX_HEIGHT - 1, CHSV(blender, 255, 168));
  else
    DRectangle(0, 0, MATRIX_WIDTH - 1, MATRIX_HEIGHT - 1, CRGB::White);
...
void audocheckers2()//22
{
  ...
  for (int16_t i = 0; i < 4; i++)
    for (int16_t qq = 0; qq < 4; qq++) {
      ...
      sizzer = constrain (sizzer, 0, 64); //size of square
      if (flop[2]) {
        DFRectangle((i + .5 - sizzer / 2 / 64.0) * radius3 , (qq + .5 - sizzer / 2 / 64.0) * radius2 , (i + .5 + sizzer / 2 / 64.0) * radius3, (qq + .5 + sizzer / 2 / 64.0) * radius2, CHSV(ccoolloorr, 255 - velo / 5, 255));
        DRectangle(i * radius3, qq * radius2, (1 + i) * radius3, (qq + 1) *radius2,  CHSV(ccoolloorr, 255 - velo / 2, 255));
      }
      else {
        DFRectangle((i + .5 - sizzer / 2 / 64.0) * radius3 , (qq + .5 - sizzer / 2 / 64.0) * radius2 , (i + .5 + sizzer / 2 / 64.0) * radius3, (qq + .5 + sizzer / 2 / 64.0) * radius2, CHSV(ccoolloorr, 255 - velo / 5, 255));
...
{
  if (counter == 0) {
    radius = mmin( MATRIX_HEIGHT / 4, MATRIX_WIDTH / 4) ;
    radius2 = mmax(MATRIX_HEIGHT, MATRIX_WIDTH) / 4 ;
    radius3 = mmax(MATRIX_HEIGHT, MATRIX_WIDTH) / 4;
  }
  if (flop[5] && flop[6])
    blender--;
  for (int16_t i = 0; i < 4; i++)
    for (int16_t qq = 0; qq < 4; qq++) {
      ...
      sizzer2 = music.raudio[(i * 4) + qq]; //brightness

      if (flop[2]) {
        // DFCircle(i * radius3 + 8, qq * radius2 + 8,  1 + radius2 / 2 , CRGB::Black);
        DFCircle((i + .5) * radius3 , (qq + .5) * radius2 , (sizzer / 64.0) * (1 + radius / 2) ,  CHSV(ccoolloorr, 255 - velo / 8, 255));
        DCircle((i + .5)* radius3 , (qq + .5) * radius2 ,  1 + radius / 2 , CHSV(ccoolloorr, 255 - velo / 2, 255));
      }
      else {
        if (flop[3]) {
          //DFCircle(i * radius3 + 8, qq * radius2 + 8,  1 + radius / 2 , CRGB::Black);
          DFCircle((i + .5) * radius3 , (qq + .5) * radius2 , (sizzer / 64.0) * (1 + radius / 2) ,  CHSV(ccoolloorr, 255 - velo / 6, 255));
          DCircle((i + .5)* radius3 , (qq + .5) * radius2 ,  1 + radius / 2 , CHSV(ccoolloorr + 85, 255 - velo / 3, 255));
        }
        else {

          //DFCircle(i * radius3 + 8, qq * radius2 + 8,  1 + radius / 2 , CRGB::Black);
          DFCircle((i + .5) * radius3 , (qq + .5) * radius2 , (sizzer / 64.0) * (1 + radius / 2) ,  CHSV(ccoolloorr, 255 - velo / 3, 255));
          //DCircle((i + .5)* radius3 , (qq + .5) * radius2 ,  1 + radius / 2 , CRGB::White);
          DCircle((i + .5) * radius3 , (qq + .5) * radius2 , (sizzer2 / 64.0) * (1 + radius / 2) ,  CHSV(ccoolloorr + 85, 255 - velo / 3, 255));
...
void audio2()// different S on big hits
{
  for (int i = 0; i < 64 ; i ++)
    for (int j = 0; j < iscalerW; j++) {
      ...
      DLine(i * iscalerW + j, LLaudio[i  ] * MATRIX_HEIGHT / 64 / 2 - 8, i * iscalerW + j, LLaudio[i  ] * MATRIX_HEIGHT / 64 / 2,  CHSV(i * mstep  + blender - 85, 196, 255));
...
void audio3()// right channel
{
  for (int i = 0; i < 64 ; i ++)
    for (int j = 0; j < iscalerW; j++) {

      DLine(i * iscalerW + j, 0, i * iscalerW + j, RRaudio[i ] * MATRIX_HEIGHT / 64 / 2, CHSV(i * mstep + blender, 255, 255));

      DLine(i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2 - 9, i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2,  CHSV(i * mstep  + blender - 64, 216, 255));

      DLine(i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2 - 6, i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2,  CHSV(i * mstep  + blender - 128, 176, 255));

      DLine(i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2 - 3, i * iscalerW + j, RRaudio[i  ] * MATRIX_HEIGHT / 64 / 2,  CHSV(i * mstep  + blender + 64, 138, 255));
    }
  audioprocess();
}
...
void spin1()  // many fingers moving in arc to middle
{
  ...
  if ((counter % (2 * BIGGER + 8))  == 0) { //-5
    dot = BIGGER / 64 + random(4, 7);
    ccoolloorr =  random8();
    hhowmany = random(3, 9);
    raad = 256 / hhowmany;
    radius2 = random(2, 5);//spin speed
  ...
  for (int q = 0; q < hhowmany; q++) {
    if (flop[3] == true) {
      ...
      if (flop[9]) {
        DFCircle(driftx + eeksangle * (2 * BIGGER  - counter % (2 * BIGGER)), drifty + whyangle * (2 * BIGGER  - counter % (2 * BIGGER)) , dot, CHSV(ccoolloorr +  h, 255 - velo / 5, 135));
        DCircle(driftx + eeksangle * (2 * BIGGER  - counter % (2 * BIGGER)), drifty + whyangle * (2 * BIGGER  - counter % (2 * BIGGER)) , dot, CHSV(ccoolloorr +  h, 255 - velo / 5, 205));
      }
      else {
        DFCircle(driftx + eeksangle * (2 * BIGGER  - counter % (2 * BIGGER)), drifty + whyangle * (2 * BIGGER  - counter % (2 * BIGGER)) , dot, CHSV(q * 255 / hhowmany + ccoolloorr +  h, 255 - velo / 5, 135));
        DCircle(driftx + eeksangle * (2 * BIGGER  - counter % (2 * BIGGER)), drifty + whyangle * (2 * BIGGER  - counter % (2 * BIGGER)) , dot, CHSV(q * 255 / hhowmany + ccoolloorr +  h, 255 - velo / 5, 205));
      }
      if (ccoolloorr % 8 == 0)
        DCircle(driftx + eeksangle * (2 * BIGGER  - counter % (2 * BIGGER)), drifty + whyangle * (2 * BIGGER  - counter % (2 * BIGGER)) , dot, CRGB::White);
    }
    else
    {
    ...
    if ( flop[0]) {
      if (counter % (2 * BIGGER) - 2 == 0)
        DFCircle(driftx  , drifty  , dot , CHSV(ccoolloorr + 21, 255 - velo / 5, 55));
      if (counter % (2 * BIGGER) - 3 == 0)
        DFCircle(driftx  , drifty  , dot - 1, CHSV(ccoolloorr + 42, 255 - velo / 5, 125));
      if (counter % (2 * BIGGER) - 4 == 0) {
        DFCircle(driftx  , drifty  , dot - 2, CHSV(ccoolloorr + 63, 255 - velo / 5, 205));
...
void spin2()  // 4 meteriorw moving in ovals
{
  ...
  DFCircle(driftx + xangle * (MIDLX / 2), drifty + yangle * (MIDLY ) , 6 + dot + dot3, CHSV(h + 2 * ccoolloorr, velo / 4 + 106, 180));
  DCircle(driftx + xangle * (MIDLX / 2), drifty + yangle * (MIDLY ) , 6 + dot + dot3, CHSV(h + 2 * ccoolloorr - 16, 255 - velo / 4, 255));
  if (!flop[8]) {
  ...
  DFCircle(driftx + xangle * (MIDLX ), drifty + yangle * (MIDLY / 2) , 6 + dot + dot3, CHSV(h , velo / 4 + 106, 180));
  DCircle(driftx + xangle * (MIDLX ), drifty + yangle * (MIDLY / 2) , 6 + dot + dot3, CHSV(h - 16, 255 - velo / 4, 255));
  if (!flop[8]) {
  ...
  DFCircle(driftx + xangle * (MIDLX / 2), drifty + yangle * (MIDLY ) , 6 + dot + dot3, CHSV(h + ccoolloorr, velo / 4 + 106, 180));
  DCircle(driftx + xangle * (MIDLX / 2), drifty + yangle * (MIDLY ) , 6 + dot + dot3, CHSV(h + ccoolloorr - 16, 255 - velo / 5, 255));
  if (!flop[8]) {
    xangle =  (sin8(2 * (h ) - 64) - 128.0) / 128.0;
    yangle =  (cos8(2 * (h ) - 64) - 128.0) / 128.0;
  }
  ...
  DFCircle(driftx + xangle * (MIDLX ), drifty + yangle * (MIDLY / 2) , 6 + dot + dot3, CHSV(h - ccoolloorr, velo / 4 + 106, 180));
  DCircle(driftx + xangle * (MIDLX ), drifty + yangle * (MIDLY / 2) , 6 + dot + dot3, CHSV(h - ccoolloorr - 16, 255 - velo / 5, 255));
}
...
void seasick6()
// full size waves overlapping
{
  if (counter == 0)
    sinewidth = random(4, 8);
  for (int jj = 0; jj < MATRIX_WIDTH; jj++) {

    for (int ii = -2 * dot2; ii < 2 * dot2 + 1 ; ii++)
      zeds(jj , ii +  MIDLY + (sin8(3 * h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.70) += CHSV(h  - 44, 255 - velo / 7, 205);
  }
  for (int jj = 0; jj < MATRIX_HEIGHT; jj++) {

    for (int ii = -3 * dot; ii < 3 * dot + 1 ; ii++)
      zeds( ii +  MIDLX + (sin8(-3 * h + jj * sinewidth) - 128) / 128.0 * MIDLX * 0.70, jj) += CHSV(h  + 44, 255 - velo / 6, 205);
  }
}
...
{
  if (counter == 0) {
    sinewidth = random(MIDLX / 8, MIDLX / 4 );
    dot2 = random(6, 13);
    dot = random(6, 13);
  }
...
// horizontal waves
{
  ...
  if (counter == 0) {
    sinewidth = random(MIDLX / 9, MIDLX / 5 );
    dot2 = random(6, 14);
    dot = random(6, 14);
  }
  ...
  for (int jj = 0; jj < MATRIX_WIDTH; jj++)
  {
    ...
    DLine(jj, (-2 + MIDLY * 3 / 2 - (sin8(2 * h + jj * (sinewidth)) - 128) / 128.0 * MIDLY * 0.50 ), jj, MATRIX_HEIGHT, CHSV(h - jj, 225, 255));
  }
  ...
  for (int jj = 0; jj < MATRIX_WIDTH; jj++) {
    if (flop[3] || flop[5])
      for (int ii = -dot2; ii < dot2 ; ii++)
        zeds(jj , ii +  MIDLY + (sin8(h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.70) = CHSV(h + jj + 128, 155 , 230);
    else
      for (int ii = -dot * 2; ii <  dot * 2 ; ii++)
        zeds( ii +  MIDLX + (sin8(h + jj * sinewidth) - 128) / 128.0 * MIDLY * 0.70, jj) = CHSV(h + jj + 128, 155  , 230);
  }
}
...
void swirl3() {
  ...
  if (flop[7])
    xhowmany = beatsin16(1, MIDLY , how, 0);
  for ( int16_t i = 0; i < xhowmany; i++) {
...
void swirl4a() {// outer
  ...
  {
    flop[9] = true;
    xhowmany = random(BIGGER / 4  , BIGGER * 2 / 3 );

    for ( int16_t i = 0; i < xhowmany; i++)
    {
      xslope[i] = random(BIGGER / 9, BIGGER / 3); //radius
      fpeed[i] = random(2, 6) * 4 - 3; //speed  bigger = slower
      fcolor[i] = blender + fpeed[i] * 23; //color
      fvelo[i] = random8();
      fcountr[i] = random8(); //position offset
    }
  }
  EVERY_N_SECONDS(29 + dot) {
    flop[9] = !flop[9];
  }

  for ( int16_t i = 0; i < xhowmany; i++) {
    if (flop[9]) {
      if (counter % int( fpeed[i]  ) == 0  && xslope[i] < BIGGER * 3 / 4  )
        xslope[i]++;
    }
    else if (counter % int( fpeed[i] ) == 0  && xslope[i] > BIGGER / 16)
      xslope[i]--;
    if (xhowmany > BIGGER) {
      if (fpeed[i] == 7) {
        DFCircle(driftx + xslope[i] * (sin8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 3, CHSV(fcolor[i]  , 130, 255));
        DFCircle(driftx + xslope[i] * (sin8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 2, CHSV(fcolor[i]  + 70, 255 - fvelo[i] / 8, 255));
      }
      else
      {
        DFCircle(driftx + xslope[i] * (sin8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 3, CHSV(fcolor[i]  , 255 - fvelo[i] / 6, 255));
        DFCircle(driftx + xslope[i] * (sin8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 2, CHSV(fcolor[i]  + 70, 255 - fvelo[i] / 6, 255));
      }
    }
    else if (i % 9 == 0) {
      DFCircle(driftx + xslope[i] * (sin8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 4, CHSV(fcolor[i]  , 130, 255));
      DFCircle(driftx + xslope[i] * (sin8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 3, CHSV(fcolor[i]  + 70, 255 - fvelo[i] / 6, 255));
    }
    else
    {
      DFCircle(driftx + xslope[i] * (sin8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 4, CHSV(fcolor[i]  , 255 - fvelo[i] / 6, 255));
      DFCircle(driftx + xslope[i] * (sin8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128 , drifty  + xslope[i] * (cos8(-h * fpeed[i] / 2  + fcountr[i]) - 128.0) / 128, 3, CHSV(fcolor[i]  + 70, 255 - fvelo[i] / 6, 255));
    }
  }
}
...
void swirl4() {// outer
  if (counter == 0)
  {
    ...
    for ( int16_t i = 0; i < xhowmany; i++)
    {
      ...
      fpeed[i] = random(2, 6) * 3 - 2; //speed
      if (flop[5])
        xpoffset[i] = blender + (fpeed[i] ) * 27; //color
      else
...
void slowmo() {//zzxzz
  EVERY_N_SECONDS(15)  {
    if (targetfps <= fastest * 2 / 3)
      targetfps = random (fastest * 2 / 3, fastest);
    else
      targetfps = random(mmin(slowest, fastest / 3), mmax(slowest, 1 + fastest / 3));
  }
}

void dualwind() {//  9   northsouth
  divider = beatsin8(dot3, MIDLY * .5, MIDLY * 1.5, 0);
  for (int16_t zx = 0; zx < MATRIX_WIDTH; zx++) {
    for (int16_t zy = 0; zy < divider; zy++) {
      zeds(zx, zy) = zeds(zx, zy + 1);
    }
    for (int16_t zy = MATRIX_HEIGHT - 1; zy >= divider; zy--) {
      zeds(zx, zy) = zeds(zx, zy - 1);
    }
  }
}

void eastwestwind() {//10  left right

  divider = beatsin8(dot3, MIDLX * .5, MIDLX * 1.5, 0);
  for (int zy = 0; zy < MATRIX_HEIGHT; zy++) {
    for (int zx = 0; zx < divider; zx++)
      zeds(zx, zy) = zeds(zx + 1, zy );
    for (int zx = MATRIX_WIDTH - 1; zx >= divider; zx--)
      zeds(zx, zy) = zeds(zx - 1, zy );
  }
}
...
void CoolBalls(int8_t Ballz) {// energy driven colors
  if (counter == 0) {
    ...
    for (int i = 0 ; i < Ballz ; i++) {
      ...
      Dampening[i] = 0.995 - 0.009 * i;
      fcolor[i] = blender + 16 * i;
      fvelo[i] = random8();
...
void bouncingmix(int8_t Ballz) {
  if (counter == 0) {
    ...
    for (int i = 0 ; i < Ballz ; i++) {
      ...
      TimeSinceLastBounce[i] = 0;
      Dampening[i] = 0.98 - 0.0075 * i;
      fcolor[i] = blender + 20 * i;
      fvelo[i] = random8();
...
void art() {
  ...
  if (flop[3] && flop[4]) {
    xvort[dot]  = beatsin8(2, xvort[dot - 1], xvort[dot + 1], 3);
    yvort[dot2] = beatsin8(3, yvort[dot2 - 1], yvort[dot2 + 1], 0);
  }
  DFRectangle(0, 0, MATRIX_WIDTH, MATRIX_HEIGHT, 0x424264);//very dim white  96 is still dim
...
{
  if (music.target && counter - hitcounter > 10) {
    phew = random8();
    hitcounter = counter;
  }
...
void noisetest() {
  ...
  if (music.peakL >= 2 || music.peakR >= 2)
  {
    if (flop[12])  gotmusic = true;
  }
...
void fakenoise() {
  if (counter == 0)
  {
    for (int16_t i = 0; i < 34; i++) {
      bpm[i] =   random(4, 25);//slow is calm and goodly.
      if (i > 0)
        while (abs(bpm[i] - bpm[i - 1]) < 5)
          bpm[i] =   random(4, 20);//slow is calm and goodly.
      if (pattern == 22 || pattern == 45 || pattern == 51)//checkers and chess
        bpm[i] = 2 * bpm[i];
    }
  }
...
void audioprocess()
{
  ...
  LLaudio[63] = (music.laudio[15] + music.laudio[16]) / 2;
  RRaudio[63] = (music.raudio[15] + music.raudio[16]) / 2;
  LLaudio[0] = (music.laudio[0] * .8 + music.laudio[1]) * .28;
  RRaudio[0] = (music.raudio[0] * .8 + music.raudio[1]) * .28;
  if (gotmusic) {
...
void fire2() {
  ...
  switch (dot) {
    ...
    case 4:
      ...
      break;
    case 5:
      thepal = CRGBPalette16( CRGB::Black, CHSV(2 * h, 255, 205), CHSV(2 * h , 128, 255), CRGB::White);
      break;
    case 6:
      thepal = pal1;
      break;
    ...
    case 7:
      ...
      thepal = CRGBPalette16( CRGB::Black, CHSV(h + 85, 255, 255), CHSV(h , 128, 255), CRGB::White);
      break;

    default:

      darkcolor  = CHSV(h, 255, 255); // pure hue,
      lightcolor = CHSV(h + 85, 128, 255); // half 'whitened'
      thepal = CRGBPalette16( CRGB::Black, darkcolor, lightcolor, CRGB::White);
      break;
...
void mindfack() {
  if (counter % BIGGER > 16 && counter % BIGGER < 42) {
    if (flop[8]) {
      if (flop[9]) {
        zeds.ShiftUp();
        zeds.ShiftUp();
...
void fillnoise8() {
  ...
  if ( Nspeed < 50) {
    dataSmoothing = 200 - (Nspeed * 4);
  }

  for (int i = 0; i < BIGGER; i++) {
    int ioffset = pscale * i;
    for (int j = 0; j < BIGGER; j++) {
      int joffset = pscale * j;
...
void noize()
{
  ...
  switch (zoomer) {
    case 0:
      //Serial.print(", Tight V zoom,  ");
      pscale = beatsin88(218, 5, 13, 0, 0);
      break;
    case 1:
      //  Serial.print(", med V zoom,  ");
      pscale = beatsin88(96, 10, 18, 0, 0);
      break;
    case 2:
      // Serial.print(", wide V zoom,  ");
      pscale = beatsin88(128, 12, 22, 0, 0);
      break;
    default:
      //no zoom
      break;
  }
  Nspeed = beatsin8(3, 2, 10);
...
void SetupRandomPalette()
{
  ...
  byte rando = random8();
  byte randy = random(24);
  currentPalette = CRGBPalette16(
                     CHSV( rando, 255, 32),
                     CHSV( rando - 12 - randy, 225 , 255),
                     CHSV( rando, 128, 255),
                     CHSV( rando + 12 + randy, 245, 255));
}
...
void SetupBlackAndWhiteStripedPalette()
{
  ...
  fill_solid( currentPalette, 16, CHSV(rando, 200, 255));
  // and set every fourth one to white.
  if (flop[7]) {
    currentPalette[0] = CRGB::Black;
    currentPalette[4] = CHSV(rando, 255, 255);
    currentPalette[12] = CRGB::Black;
  }
  else
  {
    currentPalette[0] = CHSV(rando + 24, 160, 205);
    currentPalette[4] = CHSV(rando + 48, 128, 255);
    currentPalette[2] = CHSV(rando + 24, 205, 255);
  }
  currentPalette[8] = CHSV(rando, 205, 255);
...
void randoPallette()
{
  ...
  currentPalette[0] = CHSV( h + blender, 255, 230);
  currentPalette[4] = CHSV( h + blender + 64, 245, 255);
  currentPalette[8] = CHSV( h + blender - 64, 215, 255);
  currentPalette[8] = CHSV( h + blender, 148, 255);
}

void randoPallette2()
{
  fill_solid( currentPalette, 16, CRGB::Black);
  currentPalette[0] = CHSV( blender, 255, 205);
  currentPalette[4] = CHSV( blender - 32, 190, 255);
  currentPalette[8] = CHSV( blender + 32, 215, 255);
...
void randoPallette3()
{
  fill_solid( currentPalette, 16, CHSV( h + blender, 255, 64));
  currentPalette[0] = CHSV(h + blender + 64, 255, 225);
  currentPalette[4] = CHSV(h + blender , 180, 255);
  currentPalette[8] = CHSV(h + blender - 32, 215, 255);
  currentPalette[12] = CRGB::White;
}
void BlackTealPalette()
{
  fill_solid( currentPalette, 16, CHSV( 138, 255, 60));//darkgreen
  // and set every fourth one to white.
...
{
  byte rando = random8();
  byte randy = random(148, 205);
  CRGB colorA = CHSV( rando, randy, 255);
  CRGB colorB  = CHSV( rando - random(36, 96), randy + 50, 255);
  CRGB black  = CRGB::Black;
  currentPalette = CRGBPalette16(
                     colorB,  colorB,  black,  black,
                     colorA, colorA, black,  black,
                     colorB,  colorB,  black,  black,
                     colorA, colorA, black,  black );
}
