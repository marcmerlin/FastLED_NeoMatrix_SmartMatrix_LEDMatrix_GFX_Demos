...
void audio10()//stereo display
{
  ...
  if ( flop[0])
    for (int i = 0; i < 64 ; i ++)
      for (int j = 0; j < iscalerW; j++)
      {
        DLine(i * iscalerW + j, MIDLY, i * iscalerW + j, MIDLY + LLaudio[i ] *  MATRIX_HEIGHT / 64, CHSV((i * iscalerW + j)  * mstep / dot3 +  h, 255 - velo / 4, 255));
        DLine(i * iscalerW + j, MIDLY - 1, i * iscalerW + j, MIDLY - 1 - RRaudio[i] *  MATRIX_HEIGHT / 64, CHSV((i * iscalerW + j)  * mstep / dot3 +  h, 255 - velo / 5, 255));
      }
    ...
    for (int i = 0; i < 64 ; i ++)
      for (int j = 0; j < iscalerW; j++)
      {
        DLine(i * iscalerW + j, MIDLY, i * iscalerW + j, MIDLY + LLaudio[MATRIX_WIDTH - i] * 64 / MATRIX_HEIGHT, CHSV((i * iscalerW + j) * mstep / dot3 +  h, 255 - velo / 5, 255));
        DLine(i * iscalerW + j, MIDLY - 1, i * iscalerW + j, MIDLY - 1 - RRaudio[MATRIX_WIDTH - i] * 64 / MATRIX_HEIGHT, CHSV((i * iscalerW + j)  * mstep / dot3 +  h, 255 - velo / 4, 255));
      }
  audioprocess();
...
void Raudio2()// ring, lines, colors rotate, freq does not rotate
{
  for (int i = 0; i < 64 ; i ++)
    for (int j = 0; j < iscalerW; j++) {
      ...
      DLine(MIDLX + MIDLX * xangle, MIDLY + MIDLY * yangle, MIDLX + xangle * (MIDLX - LLaudio[i ] *  MATRIX_WIDTH / 64 / (1.6 * mscale)) , MIDLY + yangle * (MIDLY - LLaudio[i ] *  MATRIX_HEIGHT / 64 / (1.6 * mscale)), CHSV((i + j) * mstep + blender + h , 255 - velo / 5, 255));
      xangle =  (sin8(h + (i * iscalerW + j) * mstep + mstep / 2 ) - 128.0) / 128.0;
      yangle =  (cos8(h + (i * iscalerW + j) * mstep + mstep / 2 ) - 128.0) / 128.0;
      DLine(MIDLX + MIDLX * xangle, MIDLY + MIDLY * yangle, MIDLX + xangle * (MIDLX - RRaudio[i] *  MATRIX_HEIGHT / 64 / (1.6 * mscale)) , MIDLY + yangle * (MIDLY - RRaudio[i] *  MATRIX_HEIGHT / 64 / (1.6 * mscale)), CHSV((i + j) * mstep + blender + h , 255 - velo / 5, 255));
    }
  audioprocess();
}
...
void Raudio7() // radiatng rotating planet pair stereo
{
  ...
  for (int i = 0; i < 64 ; i += scalerW)
    for (int j = 0; j < scalerW; j++) {
      ...
      DLine(driftx + locusx + (MIDLX / 4) * xangle, drifty + locusy + (MIDLY / 4)* yangle, driftx + locusx + xangle * (MIDLX / 4 + LLaudio[i ] * MATRIX_WIDTH / 64 / (mscale)) , drifty + locusy + yangle * (MIDLY / 4 + RRaudio[i ] * MATRIX_HEIGHT / 64 / (mscale)), CHSV((i + j) * mstep / (dot3) + blender , 255 - velo / 5, 255));
      DLine(driftx - locusx + (MIDLX / 4) * xangle, drifty - locusy + (MIDLY / 4)* yangle, driftx - locusx + xangle * (MIDLX / 4 + LLaudio[i ] * MATRIX_WIDTH / 64 / (mscale)) , drifty - locusy + yangle * (MIDLY / 4 + RRaudio[i ] *  MATRIX_HEIGHT / 64 / (mscale)), CHSV((i + j) * mstep / (dot3) + blender , 255 - velo / 5, 255));
...
void bouncingmix(int8_t Ballz) {
  ...
  for (int i = 0 ; i < Ballz ; i++) {
    switch (kind[i]) {
      case 0:
        DFCircle(Positionx[i], Positiony[i] + fcountr[i], fcountr[i],  CHSV(fcolor[i], 250, 200));
        break;
      case 1:
